...
def test_qlora_attention_no_mask_no_cache(
    ...
    err, fs_output = fs_layer_forward(sample_input, positional_embeddings=positional_embeddings)
    err.throw()
    assert_close(fs_output.attention_output, et_output)


def test_qlora_attention_with_mask(
    executorch_llama: ETTransformer,
    fartsovka_qlora_llama: QLoRALlama,
    rng_key: PRNGKeyArray,
) -> None:
...
def test_qlora_attention_with_mask_and_kv_cache(
    executorch_llama: ETTransformer,
    fartsovka_qlora_llama: QLoRALlama,
    rng_key: PRNGKeyArray,
) -> None:
