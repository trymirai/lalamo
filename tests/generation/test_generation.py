...
def test_eager_generation(language_model: LanguageModel, num_top_logits_to_return: int | None) -> None:
    ...
    token_ids, prompt_lengths = sharded_generation_batch(
        language_model,
        raw_token_ids,
    ...
    token_ids = take_first_batch_row(language_model, result.token_ids)
    ...
    if num_top_logits_to_return is not None:
        ...
        top_k_token_ids = take_first_batch_row(language_model, result.top_k_token_ids).tolist()
        top_k_token_logits = take_first_batch_row(language_model, result.top_k_token_logits).tolist()
...
def test_padding(language_model: LanguageModel) -> None:
    ...
    response_token_ids = take_first_batch_row(language_model, response_token_ids)
...
def test_batch_generation(language_model: LanguageModel) -> None:
    ...
    padded_token_ids, batched_prompt_lengths = sharded_generation_batch(
        language_model,
    ...
    response_token_ids = take_batch_prefix(language_model, response_token_ids, len(prompts))
    ...
    for pair_index, (i, j) in enumerate(pairs):
        ...
        padded, lengths = sharded_generation_batch(language_model, padded, lengths)
        ...
        result = take_batch_prefix(language_model, result, 2)
...
def test_streaming_vs_eager_consistency(replicated_language_model: LanguageModel) -> None:
    ...
    eager_token_ids, eager_lengths = sharded_generation_batch(
    ...
    eager_token_ids = take_first_batch_row(replicated_language_model, eager_token_ids)
