...
def test_standard_openai_client_chat_completions_streaming_errors_and_concurrency(
    ...
    async def run() -> None:
        ...
        async with (
            ...
            assert (
                b"".join(model.token_codec.token_bytes(token_id) for token_id in unicode_token_ids).decode() == "👩‍💻"
            )
            assert "".join(unicode_decoder.step(token_id) for token_id in unicode_token_ids) == "👩‍💻"

            protocol_token_ids = model.token_codec.encode_text("<think>private\n</think>public")
...
def test_paged_tokens_and_normalized_top20_match_dense_across_boundaries_and_arrivals(
    ...
    for model_name, prompt_lengths, output_lengths, page_size, total_pages, max_error in (
        ...
        engine = ContinuousBatchingEngine(
            model,
            ContinuousBatchingConfig(page_size=page_size, total_pages=total_pages, slot_count=2, prefill_batch_size=1),
        )
        ...
        for prompt, actual in zip(prompts, events, strict=True):
            prefilled = model.prefill_tokens(
                jnp.asarray([prompt]),
                len(prompt) + len(actual),
            ...
            for position, event in enumerate(actual, start=len(prompt)):
                ...
                max_logprob_error = max(
                    max_logprob_error,
                    abs(event.logprob - normalized[event.token_id]),
        ...
        stop_engine = ContinuousBatchingEngine(
            ...
            ContinuousBatchingConfig(
                page_size=page_size, total_pages=(prompt_lengths[0] + 2 + page_size - 1) // page_size, slot_count=1
            ),
        )
