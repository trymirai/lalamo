...
def _gated_attention_template(dtype: DTypeLike, *, has_qkv_biases: bool) -> Attention:
    initializer = EmptyInitializer(default_dtype=dtype, sharding_config=make_test_sharding_config())
...
def test_load_gated_attention_fuses_qkvg_weights_and_biases() -> None:
    module = _gated_attention_template(jnp.float32, has_qkv_biases=True)
    ...
    assert loaded.qkv_projection.output_dims == (q_dim, kv_dim, kv_dim, q_dim)
    np.testing.assert_array_equal(loaded.qkv_projection.weights.decompress(), expected_weights)
    np.testing.assert_array_equal(loaded.qkv_projection.biases, expected_biases)


def test_load_gated_attention_fuses_mlx_4bit_qkvg_weights() -> None:
    module = _gated_attention_template(jnp.bfloat16, has_qkv_biases=False)
    num_heads = module.config.num_heads
    head_dim = module.config.head_dim
    q_dim = num_heads * head_dim
    ...
    assert isinstance(loaded.qkv_projection.weights, MLXMatrixForInference)
    assert loaded.qkv_projection.weights.spec.bits == 4
    assert loaded.qkv_projection.output_dims == (q_dim, kv_dim, kv_dim, q_dim)
    np.testing.assert_array_equal(loaded.qkv_projection.weights.decompress(), expected_weights)
