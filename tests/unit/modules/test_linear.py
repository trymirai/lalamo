...
def _assert_outputs_sharded_like(outputs: tuple[Array, ...], sharding: Sharding, mesh: Mesh) -> None:
...
def _sharded_expert_inputs(values: Array) -> Array:
    return jax.device_put(values, make_sharding((ShardingAxis.EXPERT, ShardingAxis.DATA)))
...
def test_linear_matches_reference_and_splits_outputs_with_biases(fake_mesh: Mesh) -> None:
    ...
    _assert_outputs_sharded_like(outputs, inputs.sharding, fake_mesh)
...
def test_linear_under_jit_matches_reference_and_preserves_input_sharding(fake_mesh: Mesh) -> None:
    linear = _linear(_weights(), _biases())
    inputs = _sharded_input(jnp.arange(INPUT_DIM, dtype=jnp.float32))
...
def test_linear_vmapped_over_inputs_matches_reference_and_preserves_input_sharding(fake_mesh: Mesh) -> None:
    linear = _linear(_weights(), _biases())
    inputs = _sharded_batched_inputs(jnp.arange(8, dtype=jnp.float32).reshape(2, INPUT_DIM))

    outputs = jax.vmap(lambda values: linear(values, keychain=Keychain.init(3)))(inputs)
...
def test_linear_mixture_vmapped_over_experts_matches_reference_and_preserves_input_sharding(fake_mesh: Mesh) -> None:
