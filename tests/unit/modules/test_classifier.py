...
def test_prediction_head_under_jit_matches_reference_and_keeps_data_sharding(fake_mesh: Mesh) -> None:
    ...
    result = eqx.filter_jit(lambda module, values: module(values, keychain=Keychain.init(1)))(module, inputs)
...
def test_prediction_head_export_load_roundtrips_and_preserves_template_sharding(fake_mesh: Mesh) -> None:
    ...
    template = PredictionHead(
        ...
        readout=Linear(
            config=LinearConfig(),
            weights=FullPrecisionSpec().compress(dummy_array(_weights(offset=200).shape, jnp.float32)),
            biases=dummy_array((NUM_LABELS,), jnp.float32, bias_sharding),
            output_dims=(NUM_LABELS,),
        ),
    )
    ...
    result = restored(inputs, keychain=Keychain.init(3))
    reference = original(inputs, keychain=Keychain.init(4))
