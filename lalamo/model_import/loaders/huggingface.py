...
def load_huggingface_classifier(
    ...
    def load_untied_embedding_local(
        module: UntiedEmbedding,
        weights_dict: Mapping[str, Array],
        decoder_path: ParameterPath,
    ) -> UntiedEmbedding:
    ...
    # input_weights = weights_dict[decoder_path / "embeddings" / "tok_embeddings" / "weight"]
    # output_weights = weights_dict[head_path / "dense" / "weight"]
    # embedding =  load_parameters(lambda m: (m.embedding.input_weights), module, (input_weights))
    assert isinstance(module.embedding, UntiedEmbedding)
    embedding = load_untied_embedding_local(module.embedding, weights_dict, decoder_path)
    ...
    # wghts = sorted(list(weights_dict.keys()))
    # for k in wghts:
    #     print(k)
