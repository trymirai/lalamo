...
__all__ = ["DenseLogits", "Logits", "SamplingPolicy", "SparseLogits"]


_SENTINEL = -1
_MAX_BANNED_TOKENS = 16
_MAX_SPARSE_TOP_K = 128
type SamplingLeaf = Float[Array, "..."] | Int[Array, "..."]


class Logits(eqx.Module):
    values: Float[Array, "*batch support"]

    def map_values(self, function: Callable[[Array], Array]) -> Self:
        return replace(self, values=function(self.values))

    def astype(self, dtype: DTypeLike) -> Self:
        return self.map_values(lambda values: values.astype(dtype))

    def reshape(self, *shape: int) -> Self:
        return jax.tree.map(lambda array: array.reshape(*shape), self)


class DenseLogits(Logits):
    pass
...
class SamplingPolicy(eqx.Module):
    ...
    def process_logits(
        self,
        logits: Float[Array, " support"],
        token_ids: Int[Array, " support"] | None = None,
    ) -> Float[Array, " support"]:
        self._raise_if_batched()
        if token_ids is not None:
            if self.banned_tokens is not None:
                logits = jnp.where(jnp.any(self.banned_tokens[:, None] == token_ids, axis=0), -jnp.inf, logits)
            token_counts = self.token_counts
            if token_counts is not None:
                token_counts = token_counts[token_ids]
            self = replace(self, banned_tokens=None, token_counts=token_counts)  # noqa: PLW0642
    ...
    def __call__(self, logits: Logits, *, keychain: Keychain) -> Int[Array, ""]:
        token_ids = None
        policy = self
        if isinstance(logits, SparseLogits):
            token_ids = logits.token_ids
            if self.top_k is not None:
                policy = replace(
                    self,
                    top_k=eqx.error_if(
                        self.top_k,
                        self.top_k > _MAX_SPARSE_TOP_K,
                        f"SparseLogits supports top_k <= {_MAX_SPARSE_TOP_K}.",
                    ),
                )
        values = policy.process_logits(logits.values, token_ids)
        index = jax.random.categorical(keychain.vmapped_keys, values)
    ...
    def _apply_banned_tokens(
        ...
        token_ids = jnp.arange(vocabulary_size)
