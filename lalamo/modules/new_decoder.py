...
__all__ = [
    "NewDecoder",
    "NewDecoderActivationTrace",
    "NewDecoderConfig",
    "NewDecoderForwardPassConfig",
    "NewDecoderResult",
]


type NewDecoderForwardPassConfig = TransformerForwardPassConfig


class NewDecoderActivationTrace(eqx.Module):
...
class NewDecoderResult(eqx.Module):
    logits: Float[Array, "batch suffix_tokens channels"]
    updated_kv_cache: KVCache | None = None
    activation_trace: NewDecoderActivationTrace | None = None
...
class NewDecoder(LalamoModule[NewDecoderConfig]):
    ...
    def __call__(
        ...
        forward_pass_config: NewDecoderForwardPassConfig | None = None,
    ) -> NewDecoderResult:
        if token_ids.ndim != 2:
            raise ValueError(
        ...
        if return_activation_trace:
            ...
            activation_trace = NewDecoderActivationTrace(
                token_ids=token_ids,
                token_positions=token_positions,
                kv_cache=kv_cache,
