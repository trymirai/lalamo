...
from lalamo.weight_matrix import Layout, MatmulConfig, WeightMatrix
    ...
    def init_mixture(
        ...
        *,
        ...
        return Linear(
            config=self,
            sharding_config=initializer.sharding_config,
            weights=initializer.weight_matrix(
                total_output_dim,
                input_dim,
                mixture_size,
                dtype=dtype,
...
class Linear(LalamoModule[LinearConfig]):
    ...
    def input_dim(self) -> int:
        if getattr(self.weights.spec, "layout", None) == Layout.INPUT_OUTPUT:
