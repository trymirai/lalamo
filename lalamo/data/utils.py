...
def get_prompt_ending_in_user_message(conversation: Iterable[Message]) -> list[Message] | None:
    conversation = list(conversation)
    if conversation and isinstance(conversation[-1], AssistantMessage):
        conversation = conversation[:-1]
    if not conversation or not is_non_empty_user_message(conversation[-1]):
        return None
    return conversation
